...
El script incluye funciones detalladas y comentadas para los siguientes propósitos:

- **`mostrar_menu()`**  
  Muestra un menú interactivo para seleccionar una provincia desde consola.

- **`normalizar_correo(correo: str) -> str`**  
  Limpia y valida direcciones de correo electrónico. Corrige errores comunes como falta de `@`, caracteres no válidos, y dominios incompletos.

- **`corregir_correos_invalidos()`**  
  Permite al usuario revisar y corregir manualmente correos electrónicos inválidos. Asocia cada uno con su respectiva agencia y actualiza el archivo CSV.

- **`mostrar_correos_invalidos()`**  
  Muestra una lista clara y organizada de correos inválidos detectados durante el scraping.

- **`scrapear_agencias_completo(provincia: str)`**  
  Función principal que realiza el scraping del sitio web: ingresa a la web, busca por provincia, extrae los datos de cada agencia y guarda los resultados en CSV. Incluye manejo de modales y paginación.

- **`main()`**  
  Ejecuta el flujo completo, mostrando el menú, ejecutando el scraping, y ofreciendo la opción de seguir consultando otras provincias.

---

> 🔄 El código fuente está comentado línea por línea en el archivo `web_scraping-rnav_comentado.py`.
